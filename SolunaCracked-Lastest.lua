...
mainText.Text = "Discord Banned My Account for 2 Days"
...
subText1.Text = "Temporary Account: @alwaysmesmerizing"
...
subText2.Text = "I Really Hope Enjoy My Script and Join My Discord Please"
...
if DEBUG or (getfenv().Drawing and getfenv().Drawing.new) then
    ...
    function state:findTargetEntity()
        if self.targetEntityName == "" then return nil end

        if self.entityCache[self.targetEntityName] then
            local cachedChar = self.entityCache[self.targetEntityName]
            if cachedChar.Parent then 
                return cachedChar
            end
        end

        local targetPlayer = Players:FindFirstChild(self.targetEntityName)
        local character = targetPlayer and targetPlayer.Character or nil

        if character then
            self.entityCache[self.targetEntityName] = character
        end
    ...
    local function getEntityList()
            ...
            if player ~= localPlayer then
                local success, result = pcall(function()
                    ...
                    if character and character:FindFirstChild("Humanoid") then
                        ...
                        state.entityCache[player.Name] = character
                    end
                end)
                ...
                if not success then
                    ...
                    local character = player.Character
                    if character and character:FindFirstChild("Humanoid") then
                        table.insert(entityList, player.Name)
        ...
        state.entityNameMap = nameMap
        return entityList
    end

    local function updateEntityListSafely()

        if not EntityDropdown then
            warn("EntityDropdown not initialized yet")
            return
        end

        local currentTime = tick()

        if rateLimitState.isLimited then
            if currentTime - rateLimitState.lastRequestTime < rateLimitState.backoffTime then
                return
            else
        ...
        local success, result = pcall(function()
            local newList = getEntityList()
            EntityDropdown:SetValues(newList)
            return true
        end)
        ...
        if not success then
            if tostring(result):find("429") then
                ...
                rateLimitState.backoffTime = rateLimitState.backoffTime * 2 
                if rateLimitState.backoffTime > 60 then 
                    rateLimitState.backoffTime = 60 
                end
                warn("Rate limited, backing off for " .. rateLimitState.backoffTime .. " seconds")
            else
                warn("Failed to update entity list: " .. tostring(result))
            end
        else

            rateLimitState.backoffTime = 5
    ...
    local function calculateTeleportPosition(targetPart)
        local targetPos = targetPart.Position
        local targetCFrame = targetPart.CFrame

        if state.teleportMode == "Instant" then
            return CFrame.new(targetPos + Vector3.new(0, state.teleportHeight, 0))
        elseif state.teleportMode == "Behind" then
            return targetCFrame * CFrame.new(0, state.teleportHeight, state.teleportDistance)
        elseif state.teleportMode == "Smooth" then
            return CFrame.new(targetPos + Vector3.new(0, state.teleportHeight, 0))
        end
    end

    local function teleportToTarget()
        local targetEntity = state:findTargetEntity()
        ...
        local teleportCFrame = calculateTeleportPosition(targetPart)

        if state.teleportMode == "Smooth" then

            local tweenInfo = TweenInfo.new(0, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
            local tween = TweenService:Create(primaryPart, tweenInfo, {CFrame = teleportCFrame})
            tween:Play()
        else

            primaryPart.CFrame = teleportCFrame
        end
    end
    ...
    local function updateTargetESP()
        ...
        local currentTime = tick()
        if currentTime - (state.lastESPUpdateTime or 0) < 0 then
            return
        end
        state.lastESPUpdateTime = currentTime
    ...
    local function createUIComponents()
        ...
        if not Window then
            warn("Window object not found. UI library might not be initialized.")
            return false
        end
        ...
        if not success then
            warn("Failed to create Blatant tab: " .. tostring(result))
            return false
        end
        ...
        success, result = pcall(function()
            return BlatantTab:AddDropdown("EntityDropdown", {
                Title = "Choose Target",
                Values = getEntityList(),
                Default = "",
                Callback = function(Value)
                    state.targetEntityName = state.entityNameMap[Value] or getOriginalUsername(Value)
                    updateTargetESP()
                end
            })
        end)
        ...
        if not success then
            warn("Failed to create EntityDropdown: " .. tostring(result))
            return false
        end

        EntityDropdown = result

        BlatantTab:AddDropdown("TeleportMode", {
            Title = "Teleport Mode",
        ...
        BlatantTab:AddSlider("TeleportHeight", {
            Title = "Teleport Height",
            Description = "Height above target to teleport",
            Default = 5,
            Min = 0,
            Max = 20,
            Rounding = 1,
            Callback = function(Value)
                state.teleportHeight = Value
            end
        })

        BlatantTab:AddSlider("TeleportDistance", {
            Title = "Behind Distance",
            Description = "Distance behind target (for Behind mode)",
            Default = 5,
            Min = 2,
            Max = 15,
            Rounding = 1,
            Callback = function(Value)
                state.teleportDistance = Value
            end
        })
        ...
        BlatantTab:AddToggle("TargetESP", {
            Title = "Target ESP",
            Default = false,
            Callback = function(Value)
                state.targetESP = Value
                updateTargetESP()
            end
        })

        BlatantTab:AddToggle("AutoTeleport", {
            Title = "Auto Teleport",
            Default = false,
            Callback = function(Value)
                state.autoTeleport = Value
                state.lastAutoTeleportTime = tick() 
            end
        })

        BlatantTab:AddSlider("AutoTeleportInterval", {
            Title = "Auto Teleport Interval",
            Description = "Seconds between teleports",
            Default = 3, 
            Min = 0,   
            Max = 10,    
            Rounding = 1,
            Callback = function(Value)
                state.autoTeleportInterval = Value
            end
        })

        BlatantTab:AddKeybind("TeleportKeybind", {
            Title = "Teleport Keybind",
            Description = "Press key to teleport to the selected target",
            Default = "X",
            Mode = "Toggle",
            Callback = function() end,
            ChangedCallback = function(NewKey)
                state.teleportKey = NewKey
            end
        })
        ...
        BlatantTab:AddButton({
            ...
            Callback = function()
                ...
                if currentTime - (state.lastManualRefreshTime or 0) < 5 then
                    warn("Please wait before refreshing again")
                    return
                end
                state.lastManualRefreshTime = currentTime
                updateEntityListSafely()
            end
        })
    ...
    local function scheduleEntityListUpdate()

        state.entityListUpdateNeeded = true
    end

    local function setup()

        local uiSuccess = createUIComponents()
        if not uiSuccess then
            warn("Failed to create UI components. Aborting setup.")
            return false
        end
        ...
        local inputConn = UserInputService.InputBegan:Connect(function(input, gameProcessed)
            ...
            if input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode == state.teleportKey then
                inputDebounce = true

                local success, err = pcall(teleportToTarget)
                if not success then
        ...
        local playerRemovingConn = Players.PlayerRemoving:Connect(function(player)
            if player.Name == state.targetEntityName then
                state.targetEntityName = ""
                state.entityCache[player.Name] = nil

                displayNameCache[player.UserId] = nil
                updateTargetESP()
            end

            scheduleEntityListUpdate()
        end)
        ...
        local heartbeatConn = RunService.Heartbeat:Connect(function(deltaTime)
            ...
            if state.autoTeleport and state.targetEntityName ~= "" then
                if currentTime - state.lastAutoTeleportTime >= state.autoTeleportInterval then
                    state.lastAutoTeleportTime = currentTime

                    local success, err = pcall(teleportToTarget)
                    if not success then
                        warn("Auto-teleport error: " .. tostring(err))

                        if not state.consecutiveErrors then state.consecutiveErrors = 0 end
                        state.consecutiveErrors = state.consecutiveErrors + 1

                        if state.consecutiveErrors > 3 then
                            state.autoTeleportInterval = math.min(state.autoTeleportInterval * 1.5, 10)
                            warn("Increasing auto-teleport interval to " .. state.autoTeleportInterval)
            ...
            if math.floor(currentTime) % 3 == 0 and math.floor((currentTime - deltaTime)) % 3 ~= 0 then

                if state.entityListUpdateNeeded then
                    state.entityListUpdateNeeded = false

                    if EntityDropdown then
                        updateEntityListSafely()
                    end
                end

                if state.targetESP and state.targetEntityName ~= "" then
                    pcall(updateTargetESP)
                end
            end
        end)
        ...
        task.delay(1.5, function()
            if EntityDropdown then
                ...
                warn("EntityDropdown not available for initial update")
            end
        end)
    ...
    local function tryInitialize()
        ...
        if retryCount >= maxRetries then
            warn("Failed to initialize after " .. retryCount .. " attempts. Giving up.")
            return
        end

        retryCount = retryCount + 1
        warn("Initialization attempt " .. retryCount)

        local success, result = pcall(setup)

        if success and result then
            initialized = true
            warn("Successfully initialized teleport script")
        else
            warn("Initialization failed: " .. tostring(result) .. ". Retrying in 2 seconds...")

            task.delay(2, tryInitialize)
        end
    end
