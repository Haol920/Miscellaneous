...
glassFrame.Size = UDim2.new(0.8, 0, 0.6, 0)
glassFrame.Position = UDim2.new(0.1, 0, 0.2, 0)
glassFrame.BackgroundColor3 = Color3.new(1, 1, 1)
glassFrame.BackgroundTransparency = 1  
glassFrame.BorderSizePixel = 0
glassFrame.Parent = blurFrame
...
borderFrame.Position = UDim2.new(-0.002, 0, -0.002, 0)
borderFrame.BackgroundTransparency = 1
borderFrame.BorderColor3 = Color3.fromRGB(255, 255, 255)
borderFrame.BorderSizePixel = 2
borderFrame.Parent = glassFrame
...
uiGradient.Parent = glassFrame

local mainText = Instance.new("TextLabel")
mainText.Size = UDim2.new(1, 0, 0, 80)
mainText.Position = UDim2.new(0, 0, 0.25, 0)
mainText.BackgroundTransparency = 1
mainText.Text = "Script Back Up!"
mainText.TextColor3 = Color3.new(1, 1, 1)
mainText.Font = Enum.Font.GothamBold
mainText.TextScaled = true
mainText.TextTransparency = 1  
mainText.Parent = glassFrame

local subText1 = Instance.new("TextLabel")
subText1.Size = UDim2.new(1, 0, 0, 50)
subText1.Position = UDim2.new(0, 0, 0.45, 0)
subText1.BackgroundTransparency = 1
subText1.Text = "My main account @keepmesmerizing, got permanently banned :("
subText1.TextColor3 = Color3.new(1, 1, 1)
subText1.Font = Enum.Font.Gotham
subText1.TextScaled = true
subText1.TextTransparency = 1  
subText1.Parent = glassFrame

local subText2 = Instance.new("TextLabel")
subText2.Size = UDim2.new(1, 0, 0, 33)
subText2.Position = UDim2.new(0, 0, 0.70, 0)
subText2.BackgroundTransparency = 1
subText2.Text = "Love You All, Enjoy my Script, and please join my discord!"
subText2.TextColor3 = Color3.new(1, 1, 1)
subText2.Font = Enum.Font.Gotham
subText2.TextScaled = true
subText2.TextTransparency = 1  
subText2.Parent = glassFrame

local function fadeIn()
    local fadeTween = TweenService:Create(blurFrame, TweenInfo.new(1.5), {BackgroundTransparency = 0.4})
    local glassFrameTween = TweenService:Create(glassFrame, TweenInfo.new(1.5), {BackgroundTransparency = 0.6})
    local strokeTween = TweenService:Create(uiStroke, TweenInfo.new(1.5), {Transparency = 0})
    local mainTextTween = TweenService:Create(mainText, TweenInfo.new(1.5), {TextTransparency = 0})
    local subText1Tween = TweenService:Create(subText1, TweenInfo.new(1.5), {TextTransparency = 0})
    local subText2Tween = TweenService:Create(subText2, TweenInfo.new(1.5), {TextTransparency = 0})

    fadeTween:Play()
    glassFrameTween:Play()
    strokeTween:Play()
    mainTextTween:Play()
    subText1Tween:Play()
    subText2Tween:Play()
...
local function fadeOut()
    local fadeTween = TweenService:Create(blurFrame, TweenInfo.new(2), {BackgroundTransparency = 1})
    local glassFrameTween = TweenService:Create(glassFrame, TweenInfo.new(2), {BackgroundTransparency = 1})
    local strokeTween = TweenService:Create(uiStroke, TweenInfo.new(2), {Transparency = 1})
    local mainTextTween = TweenService:Create(mainText, TweenInfo.new(2), {TextTransparency = 1})
    local subText1Tween = TweenService:Create(subText1, TweenInfo.new(2), {TextTransparency = 1})
    local subText2Tween = TweenService:Create(subText2, TweenInfo.new(2), {TextTransparency = 1})
...
Configuration.TriggerBotDelay = ImportedConfiguration["TriggerBotDelay"] or 0
...
local SensitivityChanged; SensitivityChanged = UserInputService:GetPropertyChangedSignal("MouseDeltaSensitivity"):Connect(function()
    ...
    elseif not Aiming or not DEBUG and (getfenv().mousemoverel and IsComputer and Configuration.AimMode == "Mouse") then
...
Title = string.format("Soluna", string.format(MonthlyLabels[os.date("*t").month], "Soluna"), #Status > 0 and Status or " FREE "),        SubTitle = "By @ keepmesmerizing",
...
local SilentAimToggle = AimbotSection:AddToggle("SilentAim", {
    Title = "Silent Aim",
    Description = "Toggle silent aim (Spam Click)",
    Default = Configuration.SilentAim,
    Callback = function(Value)
        silentAimEnabled = Value
        Configuration.SilentAim = Value

        getgenv().SilentAimEnabled = Value

        if Value then
            if not silentAimConnection then
                loadstring(game:HttpGet("https://soluna-script.vercel.app/src/api/silent-aim.lua", true))()
...
TriggerBotSection:AddSlider("TriggerBotDelay", {
    ...
    Default = Configuration.TriggerBotDelay,
    Min = 0,  
    Max = 5,   
    Rounding = 1,
    Callback = function(Value)
        Configuration.TriggerBotDelay = Value
    end
})
    else
        ShowWarning = true
    ...
    PremiumChecksSection:AddParagraph({
        ...
        Content = " Upgrade to unlock all Options \nContact @ keepmesmerizing via Discord to buy"
    })
...
if DEBUG or (getfenv().Drawing and getfenv().Drawing.new) then
    ...
    local cache = {
        displayNames = setmetatable({}, {__mode = "k"}),
    ...
    local function formatName(player)
        if not cache.displayNames[player] then
            cache.displayNames[player] = player.DisplayName ~= player.Name 
                and player.DisplayName .. " @" .. player.Name 
                or player.Name
        end
        return cache.displayNames[player]
    end

    local function getUsername(formatted)
        return formatted:match("@(.+)$") or formatted
    end
    ...
    local function findTarget()
        if state.target.name == "" then return nil end

        local cached = cache.entities[state.target.name]
        if cached and cached.Parent then 

            if not isAlive(Services.Players:FindFirstChild(state.target.name)) then
                cache.lastTargetPosition = getPrimaryPart(cached).Position
            end
            return cached 
        end

        local player = Services.Players:FindFirstChild(state.target.name)
        local char = player and player.Character

        if char then 
            cache.entities[player.Name] = char 

            if not isAlive(player) then
                cache.lastTargetPosition = getPrimaryPart(char).Position
            end
        end
    ...
    local function getTargetPosition()
        ...
        return cache.lastTargetPosition
    end

    local function getEntities()
        local entities, nameMap = {}, {}

        for _, player in ipairs(Services.Players:GetPlayers()) do
            if player ~= localPlayer then
                local char = player.Character
                if char then
                    local formatted = formatName(player)
                    entities[#entities+1] = formatted
                    nameMap[formatted] = player.Name
                    cache.entities[player.Name] = char
                end
            end
        end
    ...
    local function updateEntityList()
        if not UI.EntityDropdown then return end

        local now = tick()

        if state.rateLimit.active then
            if now - state.rateLimit.lastRequest < state.rateLimit.backoff then return end
            state.rateLimit.active = false
        end

        if now - state.update.lastTime < state.update.interval then return end

        local success, result = pcall(function()
            UI.EntityDropdown:SetValues(getEntities())
            state.update.lastTime = now
            state.rateLimit.backoff = 5
            return true
        end)
        ...
        if not success then
            if tostring(result):find("429") then
                ...
                state.rateLimit.backoff = math.min(state.rateLimit.backoff * 2, 60)
                state.rateLimit.lastRequest = now
            end
            warn("Update failed: "..tostring(result))
        end
    end

    local function calculatePosition(targetPos)
        if not targetPos then return nil end
        return CFrame.new(targetPos + Vector3.new(0, state.teleport.height, 0))
    end

    local function teleportToPosition(position, lookAt)
        local char = localPlayer.Character
        local root = char and getPrimaryPart(char)
        if not root then return end

        local destination = lookAt and CFrame.new(position, lookAt) or CFrame.new(position)

        if state.teleport.mode == "Smooth" then
            TweenService:Create(root, TweenInfo.new(0), {CFrame = destination}):Play()
        else
    ...
    local function teleportToTarget()
        if state.orbit.enabled then return end

        local targetPos = getTargetPosition()
        if not targetPos then return end

        teleportToPosition(calculatePosition(targetPos).Position)
    end

    local function orbitTarget()
        if not state.orbit.running then
            state.orbit.running = true
            state.orbit.lastAngle = 0

            while state.orbit.enabled and state.orbit.running do
                local delta = Services.RunService.Heartbeat:Wait()
                local now = tick()

                local targetPos = getTargetPosition()
                if targetPos then
                    local center = targetPos + Vector3.new(0, state.orbit.height, 0)
                    state.orbit.lastAngle = state.orbit.lastAngle + delta * state.orbit.speed * 2 * math.pi

                    local offset = Vector3.new(
                        math.cos(state.orbit.lastAngle) * state.orbit.distance,
                        0,
                        math.sin(state.orbit.lastAngle) * state.orbit.distance
                    )

                    local desiredPos = center + offset
                    teleportToPosition(desiredPos, center)

                    cache.lastTargetPosition = targetPos
                else
    ...
    local function updateESP()
        clearESP()
        if not state.target.esp then return end
        ...
        if success then espObjects[#espObjects+1] = highlight end
    end
    ...
    local function createUI()
        ...
        local success, tab = pcall(function()
            return Window:AddTab({Title = "Blatant", Icon = "skull"})
        end)
        if not success then return false end

        UI.BlatantTab = tab

        success, UI.EntityDropdown = pcall(function()
            return tab:AddDropdown("EntityDropdown", {
                Title = "Choose Target",
                Values = getEntities(),
                Default = "",
                Callback = function(val)
                    state.target.name = cache.nameMap[val] or getUsername(val)
                    cache.lastTargetPosition = nil
                    updateESP()
                end
            })
        end)
        ...
        tab:AddDropdown("TeleportMode", {
            Title = "Teleport Mode",
            Values = {"Instant", "Smooth", "Behind"},
            Default = "Instant",
            Callback = function(val) state.teleport.mode = val end
        })

        tab:AddSlider("TeleportHeight", {
            Title = "Teleport Height",
            Default = 5, Min = 0, Max = 50, Rounding = 1,
            Callback = function(val) 
                state.teleport.height = val 
                if state.orbit.enabled then
                    state.orbit.height = val
                end
            end
        })

        tab:AddSlider("TeleportDistance", {
            Title = "Behind Distance",
            Default = 5, Min = 2, Max = 15, Rounding = 1,
            Callback = function(val) 
                state.teleport.distance = val 
                if state.orbit.enabled then
                    state.orbit.distance = val
                end
            end
        })

        tab:AddToggle("OrbitMode", {
            Title = "Orbit Mode",
            Default = false,
            Callback = function(val)
                state.orbit.enabled = val
                if val then

                    state.orbit.height = state.teleport.height
                    state.orbit.distance = state.teleport.distance
                    coroutine.wrap(orbitTarget)()
                end
            end
        })

        tab:AddSlider("OrbitSpeed", {
            Title = "Orbit Speed",
            ...
            Rounding = 1,
            Callback = function(val)
                state.orbit.speed = val
            end
        })

        tab:AddSlider("OrbitDistance", {
        ...
        tab:AddToggle("TargetESP", {
            Title = "Target ESP",
            Default = false,
            Callback = function(val) 
                state.target.esp = val 
                updateESP()
            end
        })

        tab:AddToggle("AutoTeleport", {
            Title = "Auto Teleport",
            Default = false,
            Callback = function(val) 
                state.auto.enabled = val 
                state.auto.lastTime = tick()
            end
        })

        tab:AddSlider("AutoInterval", {
            Title = "Teleport Interval",
            Default = 3, Min = 0, Max = 10, Rounding = 1,
            Callback = function(val) state.auto.interval = val end
        })

        tab:AddKeybind("TeleportKey", {
            Title = "Teleport Key",
            Default = "X",
            Callback = function() end,
            ChangedCallback = function(key)
                state.teleport.key = key
            end
        })
        ...
        tab:AddButton({
            ...
            Callback = function()
                if tick() - (state.update.lastManual or 0) < 5 then return end
                state.update.lastManual = tick()
                updateEntityList()
            end
        })
    ...
    local function setup()
        ...
        connections.input = Services.UserInputService.InputBegan:Connect(function(input, processed)
            if processed or state.teleport.debounce then return end
            if input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode == state.teleport.key then
                state.teleport.debounce = true
                pcall(teleportToTarget)
                task.delay(0, function() state.teleport.debounce = false end)
            end
        end)

        connections.playerRemoved = Services.Players.PlayerRemoving:Connect(function(player)
            if player.Name == state.target.name then

                local char = player.Character
                if char then
                    local part = getPrimaryPart(char)
                    if part then cache.lastTargetPosition = part.Position end
                end
                state.target.name = ""
                cache.entities[player.Name] = nil
                updateESP()
            end
            state.update.needed = true
        end)

        connections.playerAdded = Services.Players.PlayerAdded:Connect(function(player)
            state.update.needed = true
        ...
        connections.localCharacterAdded = localPlayer.CharacterAdded:Connect(function(char)
            if state.orbit.enabled and state.target.name ~= "" then
        ...
        connections.heartbeat = Services.RunService.Heartbeat:Connect(function(dt)
            local now = tick()

            if state.auto.enabled and state.target.name ~= "" and not state.orbit.enabled then
                if now - state.auto.lastTime >= state.auto.interval then
                    state.auto.lastTime = now

                    if not pcall(teleportToTarget) then
                        state.auto.errors = state.auto.errors + 1
                        if state.auto.errors > 3 then
                            state.auto.interval = math.min(state.auto.interval * 1.5, 10)
                            state.auto.errors = 0
                        end
                    else
                        state.auto.errors = 0
            ...
            if math.floor(now) % 3 == 0 then
                if state.update.needed then
                    state.update.needed = false
                    updateEntityList()
                end

                if state.target.esp then
                    pcall(updateESP)
                end
            end
        end)

        task.delay(1.5, updateEntityList)
        return true
    end
    ...
    local function cleanup()
        ...
        table.clear(cache.displayNames)
        table.clear(cache.entities)
        table.clear(cache.nameMap)
        cache.lastTargetPosition = nil
    end

    local maxRetries, retries = 3, 0
...
if DEBUG or (getfenv().Drawing and getfenv().Drawing.new) then
    ...
    local infiniteJumpToggle = InfiniteJumpSection:AddToggle("InfiniteJumpEnabled", {
    ...
    InfiniteJumpSection:AddKeybind("InfiniteJumpKeybind", {
        Title = "Infinite Jump Keybind",
        Description = "Press key to toggle Infinite Jump",
        Default = "J", 
        Mode = "Toggle", 
        Callback = function()
            infiniteJumpToggle:SetValue(not infiniteJumpEnabled)
        end
    })
    ...
    local walkspeedObjects = {}

    local function cleanupWalkspeedObjects()
        for _, obj in pairs(walkspeedObjects) do
    ...
    local function toggleWalkspeedModifier(enabled)
            ...
            walkspeedConnection = RunService.Heartbeat:Connect(function()
                ...
                if humanoid.MoveDirection.Magnitude > 0 then
                    ...
                    if not velocityMod then
                        ...
                        velocityMod.Name = "WalkspeedModifier"
                        velocityMod.MaxForce = Vector3.new(50000, 0, 50000) 
                        velocityMod.P = 1000
                        velocityMod.Parent = humanoidRootPart
                        table.insert(walkspeedObjects, velocityMod)
                    end

                    local moveDir = humanoid.MoveDirection.Unit
                    local speedMultiplier = currentWalkSpeed / originalWalkSpeed
    ...
    local walkspeedToggle = WalkspeedSection:AddToggle("WalkspeedEnabled", {
        Title = "Enable Speed Modifier",
        Default = false,
        Callback = function(Value)
            toggleWalkspeedModifier(Value)
        end
    })

    WalkspeedSection:AddKeybind("WalkspeedKeybind", {
        Title = "Speed Modifier Keybind",
        Description = "Press key to toggle Speed Modifier",
        Default = "V", 
        Mode = "Toggle", 
        Callback = function()
            walkspeedToggle:SetValue(not isWalkspeedEnabled)
        end
    })

    WalkspeedSection:AddSlider("WalkspeedSlider", {
        Title = "Walkspeed",
        ...
        Callback = function(Value)
            ...
            if Value ~= originalWalkSpeed and not isWalkspeedEnabled then
                walkspeedToggle:SetValue(true)
            end
            if Value == originalWalkSpeed and isWalkspeedEnabled then
                walkspeedToggle:SetValue(false)
            end
        end
    })

    WalkspeedSection:AddParagraph({
...
NoClipSection:AddKeybind("NoClipKeybind", {
    ...
    Description = "Press key to toggle No Clip",
    Default = "N",
    Mode = "Toggle",
    Callback = function()
...
local function setupFlying(character)
    ...
    local pos = Instance.new("BodyVelocity")
    pos.Name = "FlyVelocity"
    pos.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
    pos.P = 1250
    pos.Velocity = Vector3.new(0, 0, 0)
    pos.Parent = humanoidRootPart
    table.insert(flyParts, pos)

    if flyingConnection then flyingConnection:Disconnect() end
    flyingConnection = RunService.Heartbeat:Connect(function()
        local camera = workspace.CurrentCamera
        if not camera or not humanoidRootPart or not humanoidRootPart.Parent then return end

        if gyro and gyro.Parent then
            gyro.CFrame = CFrame.new(humanoidRootPart.Position, humanoidRootPart.Position + camera.CFrame.LookVector)
        end
        ...
        if pos and pos.Parent then
            pos.Velocity = moveDirection * flyingSpeed
        end
    end)
end

local function toggleFlying(enabled)
...
local flyingToggle = FlyingSection:AddToggle("FlyingEnabled", {
    Title = "Enable Flying",
    Default = false,
    Callback = function(Value)
        toggleFlying(Value)
...
FlyingSection:AddKeybind("FlyingKeybind", {
    Title = "Flying Keybind",
    Description = "Press key to toggle Flying",
    Default = "Z",
    Mode = "Toggle",
    Callback = function()
        flyingToggle:SetValue(not flyingEnabled)
    end
})

FlyingSection:AddSlider("FlyingSpeedSlider", {
    Title = "Flying Speed",
    Default = flyingSpeed,
    Min = 1,
    Max = 200,
            ...
            if getfenv().isfile(string.format("%s.Soluna", game.GameId)) and getfenv().readfile(string.format("%s.Soluna", game.GameId)) then
                ...
                for Key, Value in next, ImportedConfiguration do
                    ...
                    elseif Key == "SilentAim" then
                        SilentAimToggle:SetValue(Value)
                        Configuration.SilentAim = Value
                        silentAimEnabled = Value
                        getgenv().SilentAimEnabled = Value
...
ConfigurationManager:AddButton({
    ...
    if getfenv().setclipboard then
        DiscordWebsiteSection:AddButton({
            ...
            Callback = function()
                getfenv().setclipboard("https://discord.gg/gdpCUVj6uS")
                Window:Dialog({
                    Title = string.format(MonthlyLabels[os.date("*t").month], "Soluna"),
    ...
    if UISettings.ShowWarnings then
        ...
        elseif ShowWarning then
            ...
            Window:Dialog({
                Title = string.format("%s   PREMIUM  ", string.format(MonthlyLabels[os.date("*t").month], "Soluna")),
                Content = " Upgrade to unlock all Options  – Contact @ keepmesmerizing via Discord to buy",
                Buttons = {
                    {
                        Title = "Confirm"
                    }
...
local function IsReady(Target)
    if Target and Target:FindFirstChildWhichIsA("Humanoid") and Configuration.AimPart and 
       ...
       Player.Character:FindFirstChild(Configuration.AimPart) and Player.Character:FindFirstChild(Configuration.AimPart):IsA("BasePart") then
        ...
        if Configuration.AliveCheck and Humanoid.Health == 0 or 
           Configuration.GodCheck and (Humanoid.Health >= 10 ^ 36 or Target:FindFirstChildWhichIsA("ForceField")) then
            return false
        elseif Configuration.TeamCheck and isTeammate(_Player) then
            return false
        elseif Configuration.FriendCheck and _Player:IsFriendsWith(Player.UserId) then
            return false
        elseif Configuration.FollowCheck and _Player.FollowUserId == Player.UserId or 
               Configuration.VerifiedBadgeCheck and _Player.HasVerifiedBadge then
            return false
...
local function HandleBots()
    if not DEBUG and getfenv().mouse1click and IsComputer and Triggering and 
       (Configuration.SmartTriggerBot and Aiming or not Configuration.SmartTriggerBot) and 
       Mouse.Target and IsReady(Mouse.Target:FindFirstAncestorWhichIsA("Model")) and 
       MathHandler:CalculateChance(Configuration.TriggerBotChance) then

        local currentTime = tick()

        if Configuration.TriggerBotDelay == 0 or (currentTime - lastTriggerTime) >= Configuration.TriggerBotDelay then
            getfenv().mouse1click()
...
function TrackingHandler:DisconnectConnection(Key)
    if Key and Connections[Key] then
        for _, Connection in pairs(Connections[Key]) do
...
local function CharacterAdded(_Character)
    if typeof(_Character) == "Instance" then
        ...
        if _Player and _Player.UserId then
...
local function CharacterRemoving(_Character)
    if typeof(_Character) == "Instance" then
        for Key, Tracked in pairs(Tracking) do
    ...
    if not DEBUG and getfenv().Drawing and getfenv().Drawing.new then
        for _, _Player in pairs(Players:GetPlayers()) do
            if _Player ~= Player then
                ...
                if _Player.UserId then

                    Connections[_Player.UserId] = Connections[_Player.UserId] or {}

                    local charAddedConn = _Player.CharacterAdded:Connect(CharacterAdded)
                    local charRemovingConn = _Player.CharacterRemoving:Connect(CharacterRemoving)

                    table.insert(Connections[_Player.UserId], charAddedConn)
                    table.insert(Connections[_Player.UserId], charRemovingConn)
                end
            end
        end
    end
...
if Configuration.SilentAim then
